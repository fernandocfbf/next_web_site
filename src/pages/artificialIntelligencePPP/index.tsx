...
import functionresumeImportPPP from "../../functions/resumeImportPPP"
...
export default function ArtificialIntelligence() {
	...
	async function readExcel(file) {
		try {
			...
			promise.then((d) => {
				if (d[0]['Manchete'] == undefined) {
					
					setOpenError(true)
				}

				else {
					var d_filtered = functionresumeImportPPP(d)
	...
	function filtroData(datas) {
		...
		const final_data = functionresumeImportPPP(new_data)
	...
	async function process() {
		...
		for (var i = 0; i < dataFiltered.length; i++) {
			data_to_send.push({ "Manchete": dataFiltered[i]["Manchete"], "Link": dataFiltered[i]["Link"]})
		}
		...
		for (var i = 0; i < data_to_send.length / 40; i++) {
			await api.post('machineLearningPPP', { manchetes: data_to_send.slice(inicio, fim) })
				.then(resp => {
